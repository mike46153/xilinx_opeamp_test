...
int main(int argc, char *argv[])
{
	...
	//char rpmsg_dev[NAME_MAX] = "virtio0.rpmsg_chrdev.0.0";
	...
	for (j=0; j < ntimes; j++){
		...
		i++, size++) {
			...
			(2 * sizeof(unsigned long)) + PAYLOAD_MAX_SIZE/*size*/);
        	...
        	if((long int)tdiff_avg_ns >= 3000000){
        		printf("Index: %d, Time_avg = %lds, %ldns\n",i, (long int)tdiff_avg_s, (long int)tdiff_avg_ns);
        	}
			bytes_rcvd = read(fd, r_payload,
			(2 * sizeof(unsigned long)) + PAYLOAD_MAX_SIZE);
